...
namespace MyPG
{
	...
	void MyGameEngine::CreateEffect(int no, ML::Vec2 pos) {
		...
		case 10: //玉が広がる
			for (int i = 0; i < 6; i++) {
				if (auto e = Effect00::Object::Create(true)) {
					...
					e->addAlpha = -0.005f;
					e->countMax = 30;
					e->angle = ML::ToRadian(60 * i);
					e->moveVec = ML::Vec2(cos(e->angle), sin(e->angle));
				}
			}
			break;
		...
		case 99://「黒」フェードアウトしてからフェードイン 90フレーム
			if (auto e = Effect00::Object::Create(true)) {
			...
			break;
		case 98://「白」フェードアウトしてからフェードイン　90フレーム
			if (auto e = Effect00::Object::Create(true)) {
...
*/
	...
	MyGameEngine::MyGameEngine()
		:GameEngine(
			...
			800,							//	2D描画用解像度X(現在非対応）
			600,							//	2D描画用解像度Y(現在非対応）
			"2023年度　制作実習　スネークゲーム　「長谷川勇一朗」",	//	ウインドウタイトル
			"GPG",							//	ウィンドウクラス名
